...
struct TouchPoint
{
	public:
		...
		TouchPoint(uint8_t _id, uint16_t _x, uint16_t _y, uint16_t _size) : id(_id), size(_size), x(_x), y(_y) {};
...
{
	public:
		UM_GT911(uint8_t _addr = GT911_ADDR1);
		void begin(uint8_t _int, uint8_t _rst, uint16_t _width, uint16_t _height, bool wake = false, TwoWire *_wire = &Wire);
		void reset();
		void set_rotation(uint8_t rot);
		void refresh_resolution();
		void read(void);
		uint8_t is_detect_large;
		uint8_t touches = 0;
	...
	private:
		...
		TouchPoint read_touch_point(uint8_t *data);

		void write_bytes(uint16_t reg, uint8_t val);
		uint8_t read_bytes(uint16_t reg);
		void write_block(uint16_t reg, uint8_t *val, uint8_t size);
		void read_block(uint8_t *buf, uint16_t reg, uint8_t size);
		...
		uint16_t height;
		TwoWire *wire;
		uint8_t config_buffer[GT911_CONFIG_SIZE];
};

extern UM_GT911 touch;
